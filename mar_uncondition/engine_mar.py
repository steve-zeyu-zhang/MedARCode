...
def train_one_epoch(model, vae,
    ...
    for data_iter_step, (samples) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        valid_samples = [s.to(device, non_blocking=True) for s in samples if s.dim() == 4]
        #for i, s in enumerate(samples):
        #    print(f"Sample {i} shape: {s.shape}")
        if len(valid_samples) == 0:
            raise ValueError("No valid image samples found to process.")
        samples = torch.cat(valid_samples, dim=0)
        #samples = [s.to(device, non_blocking=True) for s in samples]
        #for i, s in enumerate(samples):
        #    print(f"after cat Sample {i} shape: {s.shape}")
